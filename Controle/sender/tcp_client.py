...
class TcpClient:
    def __init__(self, robot_ip, robot_port):
        ...
        print(f"TcpClient configurado para enviar para {self.robot_ip}:{self.robot_port}")
    ...
    def send_command(self, fl_s, fl_d, bl_s, bl_d, fr_s, fr_d, br_s, br_d, kicker):
        ...
        if self.sock is None:
            print("Erro: Nenhum socket conectado. Chame connect() primeiro")
            return
        ...
        try:
            ...
            print(f"Erro ao enviar pacote TCP para {self.robot_ip}:{self.robot_port}: {e}")

    def close(self):
        if self.sock:
            self.sock.close()
            print(f"Conexão com o robô {self.robot_ip} fechada.")
...
if __name__ == '__main__':
    ...
    client_test = TcpClient(ROBOT_TEST_IP, ROBOT_TEST_PORT)
    client_test.connect()
    print(f"\nIniciando teste de envio TCP para {ROBOT_TEST_IP}:{ROBOT_TEST_PORT}")
    ...
    try:
        ...
        client_test.sock.close()
        print("\nTeste do TcpSender concluído.")