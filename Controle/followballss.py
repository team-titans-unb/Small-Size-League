...
class Corobeu():
    ...
    def connect_CRB(self, port):
        ...
        returnCode, robot = sim.simxGetObjectHandle(clientID, 'Robot', sim.simx_opmode_blocking)

        returnCode, MotorE1 = sim.simxGetObjectHandle(clientID, 'Me1', sim.simx_opmode_blocking)
        returnCode, MotorE2 = sim.simxGetObjectHandle(clientID, 'Me2', sim.simx_opmode_blocking)
        returnCode, MotorD1 = sim.simxGetObjectHandle(clientID, 'Md1', sim.simx_opmode_blocking)
        returnCode, MotorD2 = sim.simxGetObjectHandle(clientID, 'Md2', sim.simx_opmode_blocking)
        
        returnCode, ball = sim.simxGetObjectHandle(clientID, 'ball', sim.simx_opmode_blocking)

        return clientID, robot, MotorE1, MotorE2, MotorD1, MotorD2, ball
    ...
    def Speed_CRB(self, U, omega, error_distance, Number_Iterations):
        ...
        vd = (2 * U + omega * 14) / (2 * 7.2)
        vl = (2 * U - omega * 14) / (2 * 7.2)
        a = 1
        print(f'vl === {vl} vd == {vd}')
        print(f' omega == {omega}')
        print(f' error_distance == {error_distance}')
        ### the first time #####
    ...
    def Robot_CRB(self, kpi, kii, kdi, deltaT):
        ...
        (clientID, robot, E1, E2, D1, D2, ball) = self.connect_CRB(19999)
        ...
        if (sim.simxGetConnectionId(clientID) != -1):
            ...
            while (a == 1):
                ...
                if Number_Iterations <= 1:
                    ...
                    self.phi = 0
                    sim.simxSetJointTargetVelocity(clientID, E1, 0, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, E2, 0, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, D1, 0, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, D2, 0, sim.simx_opmode_blocking)
                else:
                    ...
                    sim.simxSetJointTargetVelocity(clientID, E1, -vl, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, E2, -vl, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, D1, vd, sim.simx_opmode_blocking)
                    sim.simxSetJointTargetVelocity(clientID, D2, vd, sim.simx_opmode_blocking)
